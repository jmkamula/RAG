...
| pending | — | Ship 118' (git head from `git pull`) | `schema_v115_applicability_and_scoping_history` | pending | Ship 118' point-in-time posture reconstruction. Also carries Ship 117' architecture doc rewrite. Deploy: `bash scripts/ops/ship-118-poc-update.sh`. Adds 2 append-only audit tables (applicability_status_log + client_facts_log), new endpoint `/api/v1/admin/posture-snapshot?as_of=&fmt=json|csv|html`, and print-optimised HTML with date picker + Save-as-PDF workflow. Post-deploy: opens auditor-ledger arc (Ship 119'). |
