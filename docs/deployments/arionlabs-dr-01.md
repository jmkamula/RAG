...
| pending  | — | Ship 114' (git head from `git pull`) | `schema_v114_sector_backfill_and_check` | pending | Ship 114' sector CHECK constraint + region backfill for pre-Ship-113 tenants + strict API sector validation. Deploy: `ssh ... 'cd /data/arioncomply && git pull && bash scripts/ops/ship-114-poc-update.sh'`. schema_v114 will backfill Arion's `IT Consulting → ict_services` sector (canonical vocab). Region backfill script will normalize `country=Czechia → CZ` + set `eu_data_subjects=TRUE` derived. Strict API validation activates on restart. |
