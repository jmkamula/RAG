...
| pending | — | Ship 113'.e (git head from `git pull`) | `schema_v113_client_facts_regions_and_size_bucket` | pending | Ship 113' de-jargonized Profile scoping + region multi-select + sector controlled vocab + 3-bucket size. Adds 5 new columns (4 region bools + employee_size_bucket text). Existing tenant client_facts get FALSE defaults on the new bools; Profile UI will show "Not answered" chip until tenant fills them. Path B fix from Ship 112' already put `eu_data_subjects=true` for Arion Networks s.r.o., so their region multi-select will show EU/EEA pre-checked with "Assumed from your country" hint. |
