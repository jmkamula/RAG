...
| 2026-09-05 | operator | 68161c8d (Ship 121') | none (baseline_grants.sql extension) | GREEN | Ship 121' closed 3 of Ship 120's 4 deferred items same-session. Classified the 8 Ship-120-soft-warned tables (6 append-only compliance: audit_log/confirmation_log/deletion_log + 3 pre-COMMENTed cascade_suppression_log/client_fact_change_log/external_evidence_verification_log; 2 diagnostic: intake_consensus_log/request_trace_log). Live grant matrix on this box after deploy covers all 17 tables cleanly. Regression test 4/4. Two pre-commit guards added dev-side (not deployed to PoC): blanket-GRANT + schema_v* over-grant. Verified `deletion_log` + `rag/deletion_service.py` plumbing exists but 0 rows — partly-answers Ship 120 item 3 (erasure-with-provenance flow becomes a wire-up arc). `.deployment_log.jsonl` at 11 GREEN entries. Four arcs shipped on this box today (Ship 118' + 119' + 120' + 121'). |
| 2026-09-05 | — | 9557a561 (Ship 122') | — | N/A (dev-side only) | Ship 122' consolidates the Ship 121 audit-grant guards from pre-commit hook into `scripts/ci/check_forbidden_patterns.sh` alongside Ship 63's patterns. Also annotates 8 pre-existing historical schema_v* grants with inline `-- APPEND-ONLY-EXEMPT: <reason>` markers (7 legitimate diagnostic + 1 superseded v63 UPDATE that v79 later revoked). Also verified Neo4j ClientFact write path — one writer (offline curator loader), schema-only fields, no gap. **No PoC deploy required:** the CI script + git hook + retro doc all live dev-side; historical schema_v* annotations are cosmetic (SQL comments; runtime ignores). PoC state remains at Ship 121'. Eval baseline 237/238 PASS held identically. This box's `.deployment_log.jsonl` stays at 11 entries; no new line. Fifth arc shipped today (Ship 118'+119'+120'+121'+122') — audit-defensibility chain now complete from reconstruction through regression-prevention. |
