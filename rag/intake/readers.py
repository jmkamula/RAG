...
def _read_templated_xlsx_meta(wb) -> Optional[dict]:
    """Detect whether this workbook was generated by our xlsx template
    renderer (rag/templates/xlsx_renderer.py).

    Signature: a hidden sheet named `_arion_meta` carrying:
        leaf_id, tenant_id, column_count, column_00..column_NN, and
        (for hybrid templates) doc_field_count + doc_field_00..NN.

    Returns:
      {
        "leaf_id":       "req:A.5.9:asset_inventory",
        "tenant_id":     "<uuid>",
        "columns":       ["item:A.5.9:asset_records", ...],   # ordered
        "doc_fields":    ["item:5.3:communicated", ...],      # may be []
        "register_rows": [{col_ix: cell_text, ...}, ...],
        "doc_field_rows":[{label, standard_text, your_content}, ...],
      }
    or None if the workbook is not one of our templates.

    Cross-tenant identity check is the caller's responsibility — this
    function only reads the file. See doc_pipeline for the guard.
    """
    if "_arion_meta" not in wb.sheetnames:
        return None
    try:
        m = wb["_arion_meta"]
    ...
    if not leaf_id:
        return None  # malformed meta — bail out, let generic path handle it
...
def _read_xlsx(file_path: str, file_name: str) -> ParsedDocument:
    ...
    templated_meta = _read_templated_xlsx_meta(wb)
