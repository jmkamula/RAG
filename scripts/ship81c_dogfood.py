#!/usr/bin/env python3
"""Ship 81'.c — dogfood the LLM-curated fingerprint YAMLs on the 5
baseline docs. Snapshot as run_h_llm_signal_extract_once.csv, then score against
...
OUT_CSV = "/data/arioncomply/docs/ground_truth/ship77d_measurement/run_h_llm_signal_extract_once.csv"
    ...
    with conn.cursor() as cur:
        ...
        cur.execute(
            ...
            UPDATE document_findings df
               SET is_active = false, deletion_reason = 'ship81c_prep'
...
def reextract_all():
    t_start = time.time()
    print(f"Run H start (union+artefact, LLM signal mode (extract-once)): t_start={t_start}")
    for name, upload_id in DOCS.items():
        boundary = "----ship81c_bnd"
        body = f"--{boundary}--\r\n".encode()
        try:
            r = urlopen(Request(
    ...
    for _ in range(160):
        ...
        if len(completed) == 5:
            break
    print(f"\nRun H complete. elapsed={int(time.time()-t_start)}s")
...
if __name__ == "__main__":
    print("=== Ship 81'.c dogfood ===")
    deactivate_findings()
    reextract_all()
    snapshot_findings()
    print()
    print("Next: extend scripts/ship77e_compare.py to score run_h_llm_signal_extract_once.csv")
