#!/usr/bin/env python3
"""Ship 83'.c — dogfood the LLM-curated fingerprint YAMLs on the 5
baseline docs. Snapshot as run_j_per_must_full_curator.csv, then score against
...
OUT_CSV = "/data/arioncomply/docs/ground_truth/ship77d_measurement/run_j_per_must_full_curator.csv"
    ...
    with conn.cursor() as cur:
        ...
        cur.execute(
            ...
            UPDATE document_findings df
               SET is_active = false, deletion_reason = 'ship83c_prep'
...
def reextract_all():
    t_start = time.time()
    print(f"Run J start (union+artefact, LLM signal mode (per_must default + full curator sweep)): t_start={t_start}")
    for name, upload_id in DOCS.items():
        boundary = "----ship83c_bnd"
        body = f"--{boundary}--\r\n".encode()
        try:
            r = urlopen(Request(
    ...
    for _ in range(160):
        ...
        if len(completed) == 5:
            break
    print(f"\nRun J complete. elapsed={int(time.time()-t_start)}s")
...
if __name__ == "__main__":
    print("=== Ship 83'.c dogfood ===")
    deactivate_findings()
    reextract_all()
    snapshot_findings()
    print()
    print("Next: extend scripts/ship77e_compare.py to score run_j_per_must_full_curator.csv")
