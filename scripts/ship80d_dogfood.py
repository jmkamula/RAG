#!/usr/bin/env python3
"""Ship 80'.d — dogfood the LLM-curated fingerprint YAMLs on the 5
baseline docs. Snapshot as run_g_wired.csv, then score against
...
OUT_CSV = "/data/arioncomply/docs/ground_truth/ship77d_measurement/run_g_wired.csv"
    ...
    with conn.cursor() as cur:
        ...
        cur.execute(
            ...
            UPDATE document_findings df
               SET is_active = false, deletion_reason = 'ship80d_prep'
...
def reextract_all():
    t_start = time.time()
    print(f"Run G start (union+artefact, wired critic-verifier): t_start={t_start}")
    for name, upload_id in DOCS.items():
        boundary = "----ship80d_bnd"
        body = f"--{boundary}--\r\n".encode()
        try:
            r = urlopen(Request(
    ...
    for _ in range(160):
        ...
        if len(completed) == 5:
            break
    print(f"\nRun G complete. elapsed={int(time.time()-t_start)}s")
...
if __name__ == "__main__":
    print("=== Ship 80'.d dogfood ===")
    deactivate_findings()
    reextract_all()
    snapshot_findings()
    print()
    print("Next: extend scripts/ship77e_compare.py to score run_g_wired.csv")
